...
$wrapperUrl = "https://raw.githubusercontent.com/gradle/gradle/v8.1.4/gradle/wrapper/gradle-wrapper.jar"
...
"@ | Out-File -FilePath "$wrapperDir\gradle-wrapper.properties" -Encoding ASCII
...
exec "$JAVACMD" "$@"
"@ | Out-File -FilePath "$PSScriptRoot\gradlew" -Encoding ASCII -NoNewline

Write-Host "Gradle wrapper setup complete. You can now run './gradlew' (on Unix) or 'gradlew.bat' (on Windows)."
